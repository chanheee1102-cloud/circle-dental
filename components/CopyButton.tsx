...
export function CopyButton({ text, label = '주소 복사' }: { text: string; label?: string }) {
  ...
  return (
    <button
      ...
      onClick={copy}
      aria-live="polite"
      className="inline-flex shrink-0 items-center gap-1.5 rounded-[8px] border border-brand-300 card-glass px-3.5 py-2 text-[14px] font-semibold text-dusk transition-colors hover:border-brand-400 hover:bg-brand-50"
    >
      <span aria-hidden className="text-[13.5px]">
        {done ? '✓' : '⧉'}
      </span>
      {done ? '복사됨' : label}
    </button>
